...
class PktGen():
    ...
    def send_packet(self, src_ip, dst_ip, dst_port):
        #采用原始套接字进行数据包发送
        """ socket类型
            1.socket.AF_UNIX 用于同一台机器上的进程通信（即本机通信）
            2.socket.AF_INET 用于服务器与服务器之间的网络通信
            3.socket.AF_INET6 基于IPv6方式的服务器与服务器之间的网络通信
            4.socket.SOCK_STREAM 基于TCP的流式socket通信
            5.socket.SOCK_DGRAM 基于UDP的数据报式socket通信
            6.socket.SOCK_RAW 原始套接字，普通的套接字无法处理ICMP、IGMP等网络报文，而SOCK_RAW可以；其次        
              SOCKET_RAW也可以处理特殊的IPv4报文；此外，利用原始套接字，可以通过IP_HDRINCL套接字选项由用户构造IP头
            7.socket.SOCK_SEQPACKET 可靠的连续数据包服务
        """
        """ 
            套接字格式：socket(family,type[,protocol],协议编号默认为0)
            TCP Socket:
